...
def binary_training(true_label):
    ...
    data = data[data["main.disorder"].isin([false_label, true_label])]
    data["main.disorder"] = data["main.disorder"].map({false_label: 0, true_label: 1})
...
def predict_mood_disorder(new_data_path, true_label):
    ...
    new_data = pd.read_csv(
        new_data_path, usecols=relevant_columns[1:]
    )  # Exclude "main.disorder"
    ...
    predictions = svm.predict_proba(new_data_scaled)[
        :, 1
    ]  # Probability of Mood Disorder
...
def binary_predictions(optimal_threshold, true_label):
    ...
    output_df = pd.DataFrame(
        {"y_pred": decoded_labels, "y_true": pd.read_csv("data.csv")["main.disorder"]}
    )
